...
- **Formato de Listado de Registros:** Para enumerar los registros de un módulo y sus funciones generales, **queda prohibido el uso de tablas Markdown** (`| Registro | Función |`). Se deben utilizar exclusivamente listas con viñetas descriptivas y anidadas para detallar sus bits o funciones internas en sub-viñetas (subíndices de nivel secundario en lugar de comas seguidas en una sola línea), garantizando una excelente legibilidad vertical (ej: `* **AD1CON1:** Control principal:` seguido de sub-viñetas internas como `    * Encendido (ADON)`). Esto garantiza una mejor legibilidad y consistencia visual con el resto del documento.
